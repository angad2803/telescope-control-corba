...
const CORBA_IOR_FILE = process.env.CORBA_IOR_FILE || 'Alarm.ref';
const ENVIRONMENT = process.env.ENVIRONMENT || 'development';

console.log(`Starting Express Bridge in ${ENVIRONMENT} mode`);
...
function getAlarmDataFromCorba() {
  return new Promise((resolve, reject) => {
    ...
    const javaProcess = spawn("java", ["-cp", javaClasspath, "alarmsystem.CorbaClient"], {
      cwd: workingDir,
...
app.listen(PORT, HOST, () => {
  console.log(`Express Alarm Bridge running on ${HOST}:${PORT}`);
  console.log(`Environment: ${ENVIRONMENT}`);
  console.log(`Health check: http://${HOST}:${PORT}/health`);
  console.log(`Alarm endpoint: http://${HOST}:${PORT}/alarm`);
});
